<x-dosen.dosen-app>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('List Riwayat Dosen: ') . Auth::guard('dosen')->user()->name }}
        </h2>
    </x-slot>
                        ...
                        <div class="container px-6 py-10 mx-auto">
                            ...
                            @if (!$list_riwayat)
                                <h1
                                    ...
                                    Anda belum membuat riwayat.</h1>
                                <x-button type="submit">Tambahkan Riwayat</x-button>
                            @else
                                <div class="flex justify-between">
                                    <h1
                                        class="text-3xl font-semibold text-start text-gray-800 capitalize lg:text-4xl dark:text-white">
                                        Daftar Riwayat</h1>
                                    <x-button onclick="location.href='{{ route('riwayat.dosen.index.finished') }}'"
                                type=" submit">Tambahkan Riwayat</x-button>
                                </div>
                                <div
                                    class="grid grid-cols-1 gap-x-8 gap-y-6 mt-8 xl:mt-16 md:grid-cols-2 xl:grid-cols-2">
                                    @foreach ($list_riwayat as $riwayat)
                                        <div onclick="location.href='{{ route('riwayat.detail.dosen', $riwayat->id) }}'"
